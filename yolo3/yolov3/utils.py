...
def draw_bbox(image, ipm_image, ipm_matrix, bboxes, CLASSES=YOLO_COCO_CLASSES, show_label=True, show_confidence=True, Text_colors=(255, 255, 0), rectangle_colors='', tracking=True):
    ...
    for i, bbox in enumerate(bboxes):
        ...
        # cv2.circle(image, (round((x2+x1)/2),
        #                    round((y2-y1)*(5/6)+y1)), 3, (0, 255, 0), -1)
        # cv2.circle(image, (round((x2+x1)/2),
        #                    round((y2-y1)*(5/6)+y1)), 25, (0, 255, 0), 2)

        ori_coor = np.array([(x2+x1)/2, (y2-y1)*(5/6)+y1, 1])
        ipm_matrix = np.array(ipm_matrix)
        new_coor = np.matmul(ipm_matrix, ori_coor)
        # print("ipm_matrix : ", ipm_matrix)
        # print("new_coor_ori : ", new_coor)
        new_x = int(round(new_coor[0]/new_coor[2]))
        new_y = int(round(new_coor[1]/new_coor[2]))

        # print("ori_coor : ", round((x2+x1)/2), round((y2+y1)/2))
        # print("new_coor_xy : ", new_x, new_y)
        cv2.circle(ipm_image, (new_x, new_y),
                   10, (255, 0, 0), -1)
        # cv2.imshow('ipm', ipm)
        ...
        if show_label:
            ...
            cv2.putText(ipm_image, label, (new_x-100, new_y-50), cv2.FONT_HERSHEY_COMPLEX_SMALL,
                        2, (0, 0, 255), bbox_thick, lineType=cv2.LINE_AA)

    return image, ipm_image
...
def draw_bbox_ori(image, bboxes, CLASSES=YOLO_COCO_CLASSES, show_label=True, show_confidence=True, Text_colors=(255, 255, 0), rectangle_colors='', tracking=True):
...
def detect_video(Yolo, video_path, output_path, input_size=416, show=False, CLASSES=YOLO_COCO_CLASSES, score_threshold=0.3, iou_threshold=0.45, rectangle_colors=''):
    ...
    while True:
        ...
        image = draw_bbox_ori(original_image, bboxes, CLASSES=CLASSES,
                          rectangle_colors=rectangle_colors)
...
def detect_realtime(Yolo, output_path, input_size=416, show=False, CLASSES=YOLO_COCO_CLASSES, score_threshold=0.3, iou_threshold=0.45, rectangle_colors=''):
    ...
    while True:
        ...
        frame = draw_bbox_ori(original_frame, bboxes, CLASSES=CLASSES,
                          rectangle_colors=rectangle_colors)
